...
#include "automaton.h"
#include "automaton_int.h"
#include "automaton_int_pair.h"
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
#include <string.h>
...
{
	ssize_t cur_pos;
	for (cur_pos = 0; cur_pos < sz; cur_pos++)
	{
		u_int32_t current = mautomaton->ptr_nodes[0];
		u_int32_t where = 0;
		u_int32_t i = cur_pos;
		while ((i < sz) && !node_is_final(current))
		{
			MAFSA_letter label = l[i++];
			where = delta(mautomaton->ptr_links, current, label);
			if (where)
		...
		{
			ssize_t cpy_sz = i - cur_pos < max_out_sz ? i - cur_pos : max_out_sz;
			memcpy(out, l + cur_pos, cpy_sz);
			if (out_sz) *out_sz = cpy_sz;
			return cur_pos;
		}
	}
	return -1;
...
extern void MAFSA_automaton_search_enumerate(MAFSA_automaton mautomaton, const MAFSA_letter *l, size_t sz, MAFSA_letter *tmp, size_t tmp_size, void *user_data, MAFSA_letter delim, MAFSA_automaton_string_handler fetcher)
{
	ssize_t cur_pos;
	for (cur_pos = 0; cur_pos < sz; cur_pos++)
	{
		...
		while (i <= sz)
		{
			...
			if (where)
			{
				...
				enumerate(mautomaton, mautomaton->ptr_nodes[where], &tmp_stack, fetcher, user_data);
				stack_pop(&tmp_stack);
			}
